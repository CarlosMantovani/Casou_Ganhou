...
import { Gift, LogOut, Search, Ticket } from 'lucide-react';
...
export function AdminDashboardPage() {
  ...
  const transactionsQuery = useQuery({
    queryKey: ['admin-transactions', submittedEmailFilter, page],
    queryFn: () => adminTransactionService.list({ email: submittedEmailFilter, page, size: PAGE_SIZE }),
  });
  ...
  const submitFilter = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setPage(0);
    setSubmittedEmailFilter(emailFilter.trim());
  };
      ...
      <section className="mx-auto max-w-6xl px-6 py-8">
        ...
        <Card className="mt-6 overflow-hidden">
          ...
          {transactionsQuery.isLoading ? (
            <p className="py-10 text-center text-sm text-warm-gray">Carregando transações...</p>
          ) : null}
        ...
        <tbody className="divide-y divide-[#EEE6DF]">
          {transactions.map((transaction) => (
            <tr key={transaction.externalReference}>
              <td className="px-3 py-4 font-medium text-charcoal">{transaction.email}</td>
