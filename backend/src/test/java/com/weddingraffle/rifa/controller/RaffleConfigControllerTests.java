...
class RaffleConfigControllerTests {
    ...
    @Test
    void updatesUnitPriceForAdmin() throws Exception {
        when(raffleConfigService.updateUnitPrice(new BigDecimal("15.00")))
                .thenReturn(new RaffleConfigResponse(new BigDecimal("15.00"), null));
    ...
    @Test
    void updatesScheduledDrawAtForAdmin() throws Exception {
        ...
        mockMvc.perform(put("/admin/raffle-config/scheduled-at")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"scheduledDrawAt\":\"2026-09-05T21:00:00-03:00\"}")
                        .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
